...
export class RpcClientService {
  ...
  private async createClient(chainId: ChainId): Promise<PublicClient> {
    ...
    const rateLimitResult = await this.rpcRateLimitService.checkRateLimit(
      chainId,
    );
   ...
   */
  private async updateRateLimitAfterSuccess(
    chainId: ChainId,
    rpcUrl: string,
  ): Promise<void> {
  ...
  async execute<T>(
    ...
    for (let attempt = 0; attempt < maxFailoverAttempts; attempt++) {
      try {
        ...
        if (attempt > 0) {
          this.logger.log(
            `RPC call succeeded on chain ${chainId} after ${attempt} retry(ies) using ${
              rpcUrl || 'unknown RPC'
            }`,
          );
        }
        ...
        if (this.requiresFailover(error)) {
          ...
          if (attempt < maxFailoverAttempts - 1) {
            ...
            if (remainingRpcUrls.length > 0) {
              ...
              this.logger.debug(
                `Retrying with different RPC for chain ${chainId} (attempt ${
                  attempt + 1
                }/${maxFailoverAttempts})`,
              );
          ...
          this.logger.error(
            `All RPC endpoints exhausted for chain ${chainId} after ${
              attempt + 1
            } attempt(s)`,
          );
          throw error;
          ...
          if (rpcUrl) {
            this.rpcRotationService.recordError(
              rpcUrl,
              chainId,
              error as Error,
            );
    ...
    throw (
      lastError || new Error(`Failed to execute RPC call for chain ${chainId}`)
    );
  }
