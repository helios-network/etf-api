import { type PublicClient, parseAbi, encodeAbiParameters, decodeAbiParameters } from "viem"
import { UNISWAP_V2_FACTORY, UNISWAP_V2_ROUTER } from "../constants"
import { V2PoolInfo } from "../types/etfVerify"
 ...
 */
export async function checkV2Pool(
  client: PublicClient,
  tokenA: `0x${string}`,
  tokenB: `0x${string}`
  ...
  try {
    ...
    console.error(`Error checking V2 pool for ${tokenA}/${tokenB}:`, error)
    return { exists: false, pairAddress: null }
  }
}
...
export async function getV2Reserves(
  ...
  try {
    ...
    console.error(`Error getting V2 reserves for ${pairAddress}:`, error)
    return null
  }
}
 ...
 */
export async function calculateV2LiquidityUSD(
  client: PublicClient,
  tokenA: `0x${string}`,
  tokenB: `0x${string}`,
  ...
  try {
    const { exists, pairAddress } = await checkV2Pool(client, tokenA, tokenB)
    ...
    try {
      ...
      console.error(`Error quoting V2 swap:`, error)
      return 0
    }
  } catch (error) {
 ...
 */
export async function findV2Path(
  client: PublicClient,
  ...
  const directLiquidity = await calculateV2LiquidityUSD(
    client,
    depositToken,
    targetToken,
